...
if [[ ! -f "$DEPLOY_DIR/install/usr/local/bin/microscopi" ]]; then
    echo "ERROR: Run scripts/build-app.sh first (binary not found at deploy/install/usr/local/bin/microscopi)."
    exit 1
fi
...
echo "==> Installing custom stage3-microscopi..."
